...
export const Header: React.FC<HeaderProps> = ({ isConnected, address, handleConnect, disconnectWallet }) => {
    return (
        <header className="w-full bg-white/50 backdrop-blur-md relative z-[40] flex-shrink-0 h-[40px] sm:h-[48px]">
                ...
                <div className="flex items-center gap-2">
                    {!isConnected ? (
                        <button
                            ...
                            className="h-6 sm:h-7 w-6 sm:w-7 aspect-square min-w-[24px] min-h-[24px] sm:min-w-[28px] sm:min-h-[28px] flex-shrink-0 flex items-center justify-center bg-gradient-to-br from-[#0052FF] to-[#0040CC] text-white rounded-md shadow-[0_4px_14px_rgba(0,82,255,0.35)] hover:shadow-[0_6px_20px_rgba(0,82,255,0.45)] transition-all transform hover:-translate-y-0.5 active:scale-95"
                        >
                        ...
                        <div className="flex items-center gap-1.5 sm:gap-2">
                            ...
                            <button onClick={() => disconnectWallet()} className="w-6 h-6 sm:w-7 sm:h-7 flex items-center justify-center bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors" title="Disconnect">
